...
def main():
    ...
    print("==================================================")
    print("🌟 AI 논문 수집 및 개별 PPT 자동 생성 파이프라인 시작 🌟")
    print("==================================================\n")
    ...
    # 해외 논문을 가져오기 위해 arxiv 소스를 지정하고, 1개만 가져옵니다.
    ...
    for idx, paper in enumerate(valid_papers):
        ...
        print(f"\n▶ [{idx+1}/{len(valid_papers)}] Processing paper: {paper.get('title')}")
        
        base_name, _ = os.path.splitext(pdf_abs_path)
        summary_json_path = f"{base_name}_summary.json"
        
        # Step 2: 다단계 요약 및 수식 추출
        run_script(step2_summarize, ["--pdf", pdf_abs_path, "--force"])
        
        # Step 3: SDXL 이미지 생성 (강제 덮어쓰기 적용)
        run_script(step3_image, ["--json", summary_json_path, "--force"])
        
        # Step 4: 개별 PPT 생성
        run_script(step4_ppt, ["--json", summary_json_path])

    print("==================================================")
    print("🎉 모든 논문의 개별 PPT 생성 파이프라인 완료! 🎉")
    print(f"생성된 PPT 파일들은 {os.path.join(base_dir, 'output')} 폴더를 확인해주세요.")
    print("==================================================")
