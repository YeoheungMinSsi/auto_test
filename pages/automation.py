...
try:
    ...
    st.error(f"모듈 로드 실패 상세 오류: {e}")
    import traceback
    st.code(traceback.format_exc())
    st.stop()

def show():
    st.title("🌟 AI 논문 수집 및 PPT 자동 생성")
    st.markdown("로컬 AI 파이프라인을 실행합니다.")
    ...
    tab1, tab2, tab3 = st.tabs(["📥 파이프라인 1: 논문 수집 및 요약", "🎨 파이프라인 2: 이미지 & PPT 제작", "🗄️ Strapi DB 보관함"])
    ...
    with tab1:
        ...
        with sub_tab1:
            ...
            with col_s:
                ...
                selected_sources_1 = st.multiselect("검색 대상 사이트", options=available_sources, default=["arxiv", "scienceon"], key="s_sources_1")
            ...
            if "search_results" in st.session_state and st.session_state["search_results"]:
                ...
                for idx, paper in enumerate(results):
                    with st.container():
                        ...
                        with c2:
                            selected_cat_for_paper = st.selectbox("📂 이 논문을 저장할 분류 선택", options=cat_options, key=f"cat_select_{idx}")
        ...
        with sub_tab2:
            ...
            available_sources = ["arxiv", "scienceon", "riss", "semantic", "openalex", "pmc", "assembly"]
            selected_sources_2 = st.multiselect("검색 대상 사이트", options=available_sources, default=["arxiv", "scienceon"], key="s_sources_2")
            ...
            if st.button("🚀 자동 수집 및 AI 요약 시작", type="primary", key="btn_auto_fetch"):
                if not selected_sources_2:
                    ...
                    s1 = run_script_with_progress(
                        ...
                        f"Step 1: 논문 수집 (분류: {selected_cat})"
                    )
                        ...
                        if os.path.exists(history_file):
                            ...
                            if valid_papers:
                                ...
                                for idx, paper in enumerate(valid_papers):
                                    ...
                                    summarize_success = run_script_with_progress(
                                        step2_summarize, 
                                        ["--pdf", pdf_abs_path, "--force"], 
                                        f"Step 2: AI 다단계 요약 ({title[:20]}...)"
                                    )
                                    if not summarize_success:
                                        all_summaries_success = False
                                
                                if all_summaries_success:
                                    st.balloons()
                                    st.success("🎉 모든 수집 및 요약 작업이 완료되었습니다! 다음 탭에서 PPT를 생성하세요.")
                                else:
                                    st.error("일부 논문의 요약 과정에서 오류가 발생했습니다.")
                            else:
        ...
        if not cat_list2:
            ...
            if not paper_dict:
                ...
                if st.button("✨ 선택한 논문으로 PPT 생성 (Pipeline 2)", type="primary", key="btn_ppt"):
                    ...
                    s3 = run_script_with_progress(
                        step3_image, 
                        ["--json", selected_json_path, "--force"], 
                        f"Step 3: AI 이미지 생성 ({selected_title[:20]}...)"
                    )
                    if s3:
                        s4 = run_script_with_progress(
                            step4_ppt, 
                            ["--json", selected_json_path], 
                            f"Step 4: PPT 템플릿 생성 ({selected_title[:20]}...)"
                        )
                        if s4:
                            st.balloons()
    ...
    # ==========================================
    with tab3:
        st.header("🗄️ Strapi DB에 저장된 논문 보관함")
        st.markdown("로컬 Strapi DB에 수집되어 있는 논문 메타데이터와 요약 정보를 실시간으로 확인합니다.")
        
        try:
            strapi = StrapiClient()
        except Exception as e:
            st.error(f"Strapi 클라이언트 초기화 실패: {e}")
            strapi = None
            
        if strapi:
            if not strapi.token:
                st.warning("⚠️ `.env` 파일에 `STRAPI_API_TOKEN`이 설정되지 않았습니다. 먼저 토큰을 발급받아 등록해주세요.")
            else:
                cats = get_categories()
                filter_options = ["전체보기"] + cats
                selected_filter = st.selectbox("📂 분류(카테고리) 필터링", filter_options, key="sb_strapi_filter")
                category_arg = None if selected_filter == "전체보기" else selected_filter
                
                if st.button("🔄 DB 데이터 동기화", type="primary", key="btn_sync_db") or "strapi_papers" not in st.session_state:
                    with st.spinner("Strapi DB에서 데이터를 가져오는 중..."):
                        st.session_state["strapi_papers"] = strapi.get_papers(category=category_arg)
                
                papers_in_db = st.session_state.get("strapi_papers", [])
                
                if not papers_in_db:
                    st.info("DB에 저장된 논문이 없습니다. 먼저 탭 1에서 논문을 수집해주세요.")
                else:
                    st.success(f"DB에 총 {len(papers_in_db)}개의 논문 데이터가 존재합니다. (필터링 기준)")
                    for p in papers_in_db:
                        with st.container():
                            ...
                            with col_info:
                                ...
                                st.write(f"**저자**: {p.get('authors') or '미상'}")
                                st.write(f"**발행년도**: {p.get('published_year') or '정보 없음'}")
                                st.write(f"**분류(카테고리)**: `{p.get('category')}`")
                                st.write(f"**출처/DOI**: `{p.get('doi') or '없음'}`")
                                
                                url = p.get('source_url')
                                if url:
                                    st.markdown(f"🔗 [원문 링크 보기]({url})")
                                    
                                pdf_rel = p.get('pdf_path')
                                if pdf_rel:
                                    st.write(f"📂 **로컬 PDF 경로**: `{pdf_rel}`")
                                else:
                                    st.write("📂 **로컬 PDF**: 없음 (메타데이터만 수집)")
                                    
                            with col_summary:
                                st.write("**📝 AI 다단계 요약 내용**")
                                summary = p.get('summary')
                                if summary:
                                    st.info(summary)
                                else:
                                    st.warning("아직 요약이 진행되지 않은 논문입니다. 1-2단계에서 요약을 실행해 주세요.")
                            st.divider()
